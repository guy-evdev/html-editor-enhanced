...
class _DropdownButtonState<T> extends State<CustomDropdownButton<T>> with WidgetsBindingObserver {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null || widget.items!.isEmpty || (widget.value == null && widget.items!.where((CustomDropdownMenuItem<T> item) => item.value == widget.value).isEmpty)) {
      _selectedIndex = null;
      return;
    }
  ...
  TextStyle? get _textStyle => widget.style ?? Theme.of(context).textTheme.titleSmall;
  ...
  double get _denseButtonHeight {
    final fontSize = _textStyle!.fontSize ?? Theme.of(context).textTheme.titleSmall!.fontSize!;
  ...
  Widget build(BuildContext context) {
    ...
    final items =
        widget.selectedItemBuilder == null ? (widget.items != null ? List<Widget>.from(widget.items!) : <Widget>[]) : List<Widget>.from(widget.selectedItemBuilder!(context));
